...
const ToolInterface = ({ toolId, projectName, moderatorName }) => {
  ...
  const handleGenerateReport = () => {
    ...
    // Implémentation future de la génération de rapport
  };
      ...
      <div className="h-screen flex flex-col">
        <header className="bg-white border-b px-6 py-4">
          <div className="flex justify-between items-center">
            ...
            <div className="text-gray-600">{moderatorName} - {currentDate}</div>
          </div>
        </header>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="grid grid-cols-2 gap-8">
              ...
              >
                ...
                <p className="text-gray-600 text-center">
                  Commencez à utiliser {currentTool.name} avec les participants
                </p>
              </button>
            </div>
          </div>
        ...
        <main className="flex-1 bg-white p-4 overflow-auto">
          {mode === 'presentation' ? (
            <Suspense fallback={<div>Chargement de la présentation...</div>}>
              {React.createElement(loadToolPresentation(toolId))}
            </Suspense>
          ) : (
            <Suspense fallback={<div>Chargement de l'outil...</div>}>
              {React.createElement(loadTool(toolId), {
